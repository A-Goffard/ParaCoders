...
function renderizarProductos() {
    baseDeDatos.forEach((info) => {
        ...
        // div botones
        const miNodoBotones = document.createElement('p');
        miNodoBotones.classList.add('botonesDivSR');

        //Valor de número de productos
        const miNodoValores = document.createElement('p');
        miNodoValores.classList.add('valorprodnumero');
        miNodoValores.textContent = obtenerNumeroProductosIguales(info.id);
        ...
        miNodoBotonR.classList.add('btn', 'btn-restar');
        ...
        miNodoCardBody.appendChild(miNodoBotones);   
            miNodoBotones.appendChild(miNodoBotonR);
            miNodoBotones.appendChild(miNodoValores);
            miNodoBotones.appendChild(miNodoBoton);            
...
/**
 * Evento para restar un producto al carrito de la compra
 */
function restarProductoAlCarrito(evento) {
    // Obtenemos el marcador del producto a restar
    const marcador = evento.target.getAttribute('marcador');

    // Verificamos si el marcador es válido
    if (marcador !== null) {
        // Buscamos el índice del producto en el carrito
        const indice = carrito.indexOf(marcador);
        ...
        if (indice !== -1) {
            ...
            // Actualizamos el carrito en la interfaz de usuario
            renderizarCarrito();
        }
    }
}
...
function renderizarCarrito() {
    ...
    carritoSinDuplicados.forEach((item) => {
        ...
        miNodo.classList.add("estiloCarros");
        miNodo.textContent = `${numeroUnidadesItem} x ${miItem[0].nombre} - ${miItem[0].precio}${divisa}`;
        // Boton de borrar
        const miBoton = document.createElement('button');
        miBoton.classList.add('btn', 'btn-danger', "botonEliminarCarro");
...
/**
 * Evento para actualizar el numero de un producto en su card
 */
// Función para obtener el número de productos iguales del artículo actual en el carrito
function obtenerNumeroProductosIguales(idProducto) {
    // Filtra el carrito para obtener solo los productos del mismo tipo
    const productosIguales = carrito.filter(item => item === idProducto);
    // Devuelve el número de productos iguales del artículo actual en el carrito
    return productosIguales.length;
}
...
//para mantener el carrito si cambias de pagina
